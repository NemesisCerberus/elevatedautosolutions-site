...
const BlogPage: React.FC<BlogPageProps> = ({ navigateToPage }) => {
  ...
  const selectPost = (index: number) => {
    const slug = blogPosts[index]?.slug;
    if (slug) {
      history.pushState({ page: 'blog', post: slug }, '', window.location.pathname + `#/blog/${slug}`);
    }
    setSelectedPost(index);
    window.scrollTo(0, 0);
  };

  // Read hash on mount to restore a directly-linked blog post
  React.useEffect(() => {
    const hash = window.location.hash;
    if (hash.startsWith('#/blog/')) {
      const slug = hash.replace('#/blog/', '');
      const idx = blogPosts.findIndex(p => p.slug === slug);
      if (idx >= 0) setSelectedPost(idx);
    }
  }, []);
  ...
  const blogPosts = [
    {
      id: 1,
      slug: 'how-gamified-learning-increases-student-engagement',
...
Gamification works because it meets students where they are. It respects their attention span. It gives them immediate feedback instead of waiting weeks for a test grade. And yes—it makes learning feel less like punishment and more like something worth their time.`
    ...
    {
      id: 2,
      slug: 'expanding-the-pathway-traditional-training-flexible-learning',
...
The talent is there. The drive is there. We just need systems that work for different life situations, not just the ideal ones.`
    ...
    {
      id: 3,
      slug: 'from-technician-to-educator-why-skilled-instructors-need-better-tools',
...
The technicians transitioning into education aren't lacking skill. They're lacking support. Let's give them the tools they deserve.`
    ...
    {
      id: 4,
      slug: 'ai-wont-replace-technicians-itll-make-them-superpowers',
...
The answer to that question will determine which shops and training programs thrive over the next five years.`
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--navy-deepest)' }}>
      <section style={{ padding: '80px 0 40px' }}>
        <div className="container">
          ...
          <div style={{ maxWidth: '900px' }}>
            <h1 style={{ fontSize: '52px', fontWeight: 900, marginBottom: '16px', color: 'var(--text-primary)', letterSpacing: '-1.5px' }}>
              From the Blog
            </h1>
              ...
              <article>
                <div style={{ marginBottom: '24px' }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{currentPost.date}</span>
                  </div>
                ...
                {blogPosts.map((post, index) => (
                  <button
                    ...
                    style={{
                      padding: '16px',
                      borderRadius: '12px',
                      background: selectedPost === index ? 'rgba(59, 130, 246, 0.15)' : 'transparent',
                      border: selectedPost === index ? '2px solid rgba(59, 130, 246, 0.4)' : '2px solid var(--border-subtle)',
                      cursor: 'pointer',
                      textAlign: 'left',
                      transition: 'all 0.3s',
                      color: selectedPost === index ? 'var(--text-primary)' : 'var(--text-secondary)',
                      fontFamily: 'inherit'
                    }}
                    onMouseOver={(e) => {
                  ...
                  >
                    <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--accent-cyan)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                      {post.category}
                    </div>
